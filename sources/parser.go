...
func readAvailableTiles(gameMap string, cellImages []*MyImage) {
	...
	for i < len(mapLines) {
		...
		cell := &Cell{img.image, &Transform{
			Point{0.0, 0.0},
			Point{0.0, 0.0}, 0.0, 1.0, 1.0},
			cellType, false, &MovementPlaceHolder{
			Point{0.0, 0.0},
	...
	for i := 0; i < len(gameMap); i++ {
		...
		if isTile(cellType) {
			tiles = append(tiles, &Tile{ img.image, &Transform{
				...
				Point{float64(col), float64(row)}, 0.0, 1.0, 1.0},
				cellType, false})
		} else {
			cell := &Cell{ img.image, &Transform{
				Point{float64(col), float64(row)},
...
func loadMapFromFile() {
	...
	data, err := os.ReadFile("./maps/map.map")
	if err != nil {
		log.Fatal(err.Error())
		panic(err)
	}
